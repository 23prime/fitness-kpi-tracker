...
- [x] ローカル転送での振る舞いベース検証（データ書き込み→`.db`単体では未反映を確認→バックアップ→アンインストール／再インストールで復元→データが存在することを確認）で、`-wal` がバックアップ対象に含まれ復元後に正しく適用されることを確認する。取りこぼしは発生せず、`TRUNCATE` へのジャーナルモード変更は不要だった。
- [x] 要件定義書の `backup_rules.xml` の記述を `data_extraction_rules.xml` に合わせて修正する。
- [x] バックアップの手順と制約を `docs/backup.md` に記載し、README からリンクする。
...
| 箇所 | 内容 |
| --- | --- |
| バックアップ設定ファイル | 要件定義書は `backup_rules.xml` としていたが、minSdk 34 では `android:fullBackupContent`（`backup_rules.xml`）は参照されず、`android:dataExtractionRules`（`data_extraction_rules.xml`）のみが有効である。#12 で要件定義書を修正済み。 |
...
- ~~WAL を含む Auto Backup の復元が実機で成立するか（#12）。~~ → 実機検証済み。ローカル転送でのバックアップ・復元で `-wal` の未チェックポイント分が正しく復元されることを確認した。
