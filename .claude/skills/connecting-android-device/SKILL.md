...
- **`run-as` can read app-private files but not create or overwrite them.** `run-as <pkg> cat databases/foo.db` works; `run-as <pkg> sh -c 'cat src > databases/foo.db'` fails with `No such file or directory` even when `databases/` exists and is writable by that UID (confirmed with a plain `echo test > databases/testfile.txt`, same failure). This looks like an SELinux restriction on the shell domain that survives the `run-as` UID switch, not a DAC permission problem, and no number of retries fixes it. Do not use `run-as ... cat > file` or `adb push` + `run-as cat` to modify app data on this device. To change app data, use the app's own UI, or restore via the app's Auto Backup path (uninstall, then reinstall against a transport with a known-good backup). If a write into app-private storage fails partway (e.g. after deleting the target file to retry), stop and reassess before doing anything else — in particular, never launch the app with its data file missing or partially written; Room/SQLite will silently create a fresh empty database and the prior data is gone.
